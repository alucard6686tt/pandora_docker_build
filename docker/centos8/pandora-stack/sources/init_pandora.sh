...
if [[ $OPEN != 1 ]]; then
	...
	#install only open
	dnf localinstall --disablerepo=* /opt/pandora/pandorafms_console-*.rpm /opt/pandora/pandorafms_server-*.rpm
	dnf reinstall --disablerepo=* /opt/pandora/pandorafms_server-*.rpm
	[[ -e /var/www/html/pandora_console/include/config.php ]] || dnf reinstall --disablerepo=* /opt/pandora/pandorafms_console-*.rpm
